...
# Step 2: Compute the corrective transformation (G) using Basis Constraints
def compute_corrective_transformation_basis_constraints(M_tilde, B_tilde, F, K):
    Q = np.zeros((3 * K, 3 * K))  # transformation constraints

    for k in range(K):  # Use the first K frames as bases
        Mf = M_tilde[2 * k : 2 * k + 2, :]  # Extract rows for each selected frame
        Q += np.dot(Mf.T, Mf)

    # Solve for G via SVD of Q
    U, _, Vt = svd(Q)
    G = U @ Vt  # make G orthonormal
...
def main(W, M_true, B_true):
    ...
    G = compute_corrective_transformation_basis_constraints(M_tilde, B_tilde, F, K)

    # Recover motion and shape from the corrective transformation and factored measurement matrix
