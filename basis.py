...
def compute_corrective_transformation_basis_constraints(M_tilde, B_tilde, F, K):
    # Select the first K frames to serve as independent bases
    selected_frames = np.arange(K)
    Q = np.zeros((3 * K, 3 * K))  # Corrective transformation constraints

    for k in selected_frames:
        Mf = M_tilde[2 * k : 2 * k + 2, :]  # Extract rows for each frame
        Q += np.dot(Mf.T, Mf)
...
def main(W,M_true,B_true):
    ...
    # Compute the corrective transformation using Basis Constraints
